    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        dispatcher.register(Commands
                ...
                .then(Commands.literal("reload").executes(context -> {
                            context.getSource().sendSuccess(new TranslationTextComponent(LangKeys.COMMANDS_RELOAD_START.key()), true);
                            NameManager.INSTANCE.reloadRemoteLinks().thenAccept(change -> {
                                context.getSource().sendSuccess(new TranslationTextComponent(LangKeys.COMMANDS_RELOAD_DONE.key(), change), true);
                            });
                            return 1;
                        })
    ...
    private static int spawnPlayerMob(CommandSource source, String username, Vector3d pos) throws CommandSyntaxException {
        ...
        if (!World.isInSpawnableBounds(blockpos)) {
            ...
            if (entity == null) {
                ...
                entity.finalizeSpawn(source.getLevel(), source.getLevel().getCurrentDifficultyAt(entity.blockPosition()), SpawnReason.COMMAND, null, null);
                entity.setUsername(username);

                if (!source.getLevel().tryAddFreshEntityWithPassengers(entity)) {
                    throw DUPLICATE_UUID.create();
                }
                source.sendSuccess(new TranslationTextComponent(LangKeys.COMMANDS_SPAWN_SUCCESS.key(), entity.getDisplayName()), true);
                return 1;
            }
        }
    }
